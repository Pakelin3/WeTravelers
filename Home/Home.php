...
<body id="Cuerpito">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            // Array de todos los posts
            var allPosts = [
                "Post 1",
                "Post 2",
                // Aquí van los demás posts
            ];
            var visiblePosts = 18; // Número de posts que se muestran inicialmente
            var postsContainer = document.getElementById("post-container");

            function displayPosts(startIndex, endIndex) {
                for (var i = startIndex; i < endIndex; i++) {
                    var postElement = document.createElement("div");
                    postElement.textContent = allPosts[i];
                    postsContainer.appendChild(postElement);
                }
            }
            // Mostrar los primeros 18 posts
            displayPosts(0, visiblePosts);
            var loadMoreButton = document.getElementById("load-more-btn");
            loadMoreButton.addEventListener("click", function() {
                var nextBatch = visiblePosts + 18; // Cargar 18 posts adicionales
                if (nextBatch >= allPosts.length) {
                    // Si no hay más posts por cargar, ocultar el botón
                    loadMoreButton.style.display = "none";
                }
                displayPosts(visiblePosts, nextBatch);
            ...
            <div class="card-body position-relative">
                ...
                <input type="text" class="form-control comentario" placeholder="Escriba un comentario...">
                        ...
                        $('.heart-container').click(function() {
                            ...
                            // Realizar cualquier otra lógica necesaria aquí, como enviar datos al servidor para registrar el like
