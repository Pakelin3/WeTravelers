...
if (!isset($_SESSION['id_usuario'])) {
    ...
    exit(); // Terminar el script para evitar que se ejecute mas codigo
}
?>
...
<body id="Cuerpito">
    ...
    <nav class="navbar navbar-expand-lg bg-body-tertiary fixed-top" data-bs-theme="dark" style="height: 75px;">
        <div class="container-fluid">
            ...
            <div id="navbarSupportedContent" class="collapse navbar-collapse">
                ...
                <script>
                    (function($) {
                        $(document).ready(function() {
                            cargarDatosAdmin
                                (); // Llamada para cargar los datos del administrador al cargar la pagina
                ...
                <!-- Aqui se mostrara la imagen y el nombre del usuario -->
            </div>
        </div>
    </nav>
    <script>
        // Utiliza una funcion autoinvocada para asegurar que el codigo se ejecute cuando jQuery este disponible
        (function($) {
            // Tu codigo jQuery va aqui dentro
            $(document).ready(function() {
                cargarDatosUsuario(); // Llamada inicial para cargar los datos del usuario al cargar la pagina
        ...
        })(jQuery); // Pasa jQuery como argumento para asegurar que $ sea una referencia a jQuery dentro de la funcion
    </script>
    ...
    <div id="panel-lateral" class=" flex-column flex-shrink-0 p-3 text-bg-dark fixed-top z-1" style="height: calc(100vh - 75px ); width: 280px; top:75px; display: flex; justify-content: space-between;">
            ...
            <div id="plist" class="people-list">
                <ul id="chatList" class="list-unstyled chat-list mt-2 mb-0">
                    ...
                    <li class="clearfix">
                        ...
                        <div class="about">
                            <div class="name">Moises Gomez</div>
        ...
        <a href="../BDD_Conexion/Cerrar_sesion.php" class="cerrar_sesion order-1 btn btn-danger">
            ...
            <span class="texto-boton">Cerrar sesion</span>
        </a>

    </div>

    <div id="feed-container" class="d-flex flex-wrap justify-content-around">
        <!-- Aqui se cargaran las publicaciones -->
    </div>
    <script>
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $.each(response, function(index, publicacion) {
                            // Crear el HTML para una publicacion
                            var html = `
                    <div class="card">
                        <div class="card-header">
                            ...
                            <button type="button" class="btn btn-link" id="ubicacion_publicacion">${publicacion.ubicacion}</button>
                        </div>
                        ...
                        <div class="card-body position-relative">
                            ...
                            // Agregar la publicacion al contenedor de alimentacion
                            $('#feed-container').append(html);
                        });
                        // Manejar el evento clic en el contenedor del corazon despues de agregar todas las publicaciones
                        $('.heart-container').click(function() {
                            // Obtener el ID de la publicacion asociada
                            var publicacionId = $(this).data('publicacion-id');
                            // Almacenar una referencia al contenedor del corazon
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                    // Si se guardo el like correctamente, actualizar el contador de likes
                                    if (response.success) {
                            ...
                            // Alternar la clase 'liked' para cambiar el color del corazon
            ...
            // Llamar a la funcion para cargar las publicaciones cuando el documento este listo
            cargarPublicaciones();
        });
    </script>

    <!--Modal crear publicacion-->
    <div class="modal fade" id="Crear_publicacion" tabHome="0" role="dialog" aria-labelledby="Crear_publicacionCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-body">
                    <form action="">
                        <input type="text" class="form-control" id="titulo" placeholder="Ponle un titulo" style="margin-bottom: 15px;">
                        <textarea class="form-control" id="descripcion" placeholder="Añade una descripcion" style="resize: none;"></textarea>
                        <div class="button_outer btn btn-light btn-sm" style="margin-top: 10px;">
                            <div class="btn_upload subir_foto">
                                ...
                                <input type="file" class="upload_file" name="" accept=".gif, .png, .jpg, .jpeg">
                            </div>
                        </div>
                        <div class="uploaded_file_view"></div>
                    </form>
                    <input type="text" id="ubicacion_input" name="ubicacion" value="" readonly value="" style="border: none;">
    ...
    <script>
        $(document).on("click", "#Crear_publicacion .btn-success", function() {
            // Obtener los valores del formulario
            var titulo = $("#titulo").val();
            var descripcion = $("#descripcion").val();
            var imagen = $(".upload_file").prop('files')[0];

            // Verificar que el titulo, la descripcion y la imagen tengan contenido
            if (titulo.trim() !== "" && descripcion.trim() !== "" && imagen) {
                ...
                if ($.inArray(extension, ['gif', 'png', 'jpg', 'jpeg']) == -1) {
                    ...
                    $("#mensaje_publicacion").html(
                        '<div class="alert alert-danger" role="alert" style="margin-top: 15px;">Por favor, seleccione una imagen o un GIF válido.</div>'
                    );
                ...
                formData.append('ubicacion', $("#ubicacion_input").val()); // Agregar la ubicacion
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $("#mensaje_publicacion").html(
                            '<div class="alert alert-success" role="alert" style="margin-top: 15px;">Publicacion creada exitosamente.</div>'
                        );
                        // Si el mensaje es "Publicacion creada exitosamente", cerrar el modal despues de 2 segundos
                        if (response.trim() === "Publicacion creada exitosamente.") {
                            setTimeout(function() {
                ...
                // Limpiar mensajes anteriores
                $("#mensaje_publicacion").html('');
                // Mostrar una alerta si alguno de los campos esta vacio
                $("#mensaje_publicacion").html(
    ...
    <script type="text/javascript">
        $.getJSON('http://ipwhois.app/json/', function(ip) {
            ...
            // Concatenar pais y estado
            var ubicacion = ip.country + '/' + ip.region;
            // Mostrar ubicacion en el formulario
            $('#ubicacion_input').val(ubicacion);
        });
    </script>


    <!-- Modal ver perfil -->
    <div class="modal fade" id="Ver_perfil" tabHome="-1" role="dialog" aria-labelledby="Ver_perfilLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="Ver_perfilLabel">Perfil de Usuario</h5>
                    <img src="" id="imagen_usuario" alt="Imagen del usuario" style="width: 50px; height: 50px; border-radius: 50%; margin-left:10px;">
                </div>
                <div class="modal-body">
            ...
            $('#Ver_perfil').on('shown.bs.modal', function() {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#nombre_usuario').text(data.nombre_usuario);
                        $('#correo').text(data.correo);
                        $('#continente').text(data.continente);
                        $('#pais').text(data.pais);
                        $('#estado').text(data.estado);
                        // Ajusta la ruta de la imagen y establece el atributo src
                        $('#imagen_usuario').attr('src', '../rsc/uploads/' + data.imagen);
                    },
                    error: function(xhr, status, error) {
                        console.error(error);
                    }
